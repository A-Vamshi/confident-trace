...
| Google GenAI | generate_content; generate_content_stream; stands down while OpenTelemetry's GoogleGenAiSdkInstrumentor is instrumented | Sync, async, streaming; mocked SDK transports; simulated upstream instrumentor in both wrap orders |
...
| Existing OTel instrumentation | Export selection on shared TracerProvider: Confident/GenAI spans plus their open local ancestors; export_all_spans=True passes everything through | Original IDs, parents, attributes, events and schema preserved on exported spans; shared span-export vectors; framework-specific coverage listed separately |
...
- Native adapter versions listed above are tested versions, not installation or runtime restrictions. Other versions may work but are unverified; missing AgentCore middleware capabilities leave existing native export and application behavior unaffected. No universal duplicate detection; disable overlapping provider instrumentation for unverified native frameworks. The google_genai integration stands down while OpenTelemetry's GoogleGenAiSdkInstrumentor is instrumented; when that instrumentor's span encloses ours it stands down only on the same provider, otherwise without checking which provider the instrumentor exports to.
- Unknown fields and enum values in exported third-party spans pass through unchanged. By default, spans without Confident or GenAI data are exported only as ancestors of exported spans; OpenInference-only spans need export_all_spans=True. Registry completeness is not an implementation-coverage claim.
