...
| LiveKit Agents (optional) | Native OTel export and JobContext._on_cleanup lifecycle hook; no runtime version gate | 1.8.3 with livekit-plugins-openai 1.8.3 and OpenAI 2.54.0 |
...
| Existing OTel instrumentation | Confident/GenAI spans and their parents on shared TracerProvider; export_non_ai_spans=True passes everything | Original IDs, parents, attributes, events and schema preserved; framework-specific coverage listed separately |
...
| LiveKit Agents (native OTel) | AgentSession voice and text turns, LLM requests, tools and lifecycle spans; overlapping model suppression | Real AgentSession with the OpenAI plugin and mocked HTTP/SSE; labels, one span per model call, parentage, unset/configured LiveKit provider, deselection, both privacy import orders, and spawn/forkserver worker cleanup delivery. |
