...
- Native adapter versions listed above are tested versions, not installation or runtime restrictions. Other versions may work but are unverified; missing AgentCore middleware capabilities leave existing native export and application behavior unaffected. No universal duplicate detection; disable overlapping provider instrumentation for unverified native frameworks. The google_genai integration stands down whenever OpenTelemetry's GoogleGenAiSdkInstrumentor is instrumented, without checking which provider that instrumentor exports to.
