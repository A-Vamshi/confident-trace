...
| LiveKit Agents (optional) | Native OTel export and inference recognition; no runtime version gate | 1.8.3 with livekit-plugins-openai 1.8.3 and OpenAI 2.54.0 |
...
| LiveKit Agents (native OTel) | AgentSession voice and text turns, LLM requests, tools and lifecycle spans; overlapping model suppression | Real AgentSession with the OpenAI plugin and mocked HTTP/SSE; labels, one span per model call, parentage, unset/configured LiveKit provider and deselection. |
